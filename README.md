# Fedora 38 Ansible Test Image with non-root user

Fedora 38 Docker container for Ansible playbook and role testing and non-root user _ansible_.
...
1. Run `docker build -t fedora38-ansible-nonroot .`

### Podman

1. [Install Podman]().
1. `cd` into this directory.
1. Run `podman build -t fedora38-ansible-nonroot .`.

## How to Use

1. Install Docker or Podman.
1. Build the image.
1. Run a container from the image `podman run --detach --privileged  --volume=/sys/fs/cgroup:/sys/fs/cgroup:rw --cgroupns=host localhost/fedora38-ansible-nonroot`. For testing of Ansible roles I integrate this into molecule, but you can mount the current directory into the container with ``---volume=`pwd`:/etc/ansible/roles/testing:ro``).
...
Based on [docker-fedora38-ansible](https://github.com/geerlingguy/docker-fedora38-ansible) by [Jeff Geerling](https://www.jeffgeerling.com/).
